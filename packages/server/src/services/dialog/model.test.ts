...
describe('chooseModel', () => {
  ...
  it('вопрос о конкретном комплексе поднимает на старшую модель', () => {
    // Живые вопросы заказчика: ответ на них собирается из карточки объекта
    // в базе знаний, а младшая модель обходилась одним list_projects.
    expect(escalationReason('где жк восточный')).toBe('вопрос о комплексе')
    expect(escalationReason('расскажи про Серебро')).toBe('вопрос о комплексе')
    expect(escalationReason('что рядом с Красной горкой')).toBe('вопрос о комплексе')
    expect(escalationReason('какой адрес у Космоса')).toBe('вопрос о комплексе')
    expect(escalationReason('когда сдача Серебра')).toBe('вопрос о комплексе')
  })

  it('подбор со словом «где» комплексом не считается', () => {
    // «Где студии до 5 млн» — это поиск, а не вопрос о доме.
    expect(escalationReason('где студии')).toBeNull()
  })
