 ...
 *
 * Три поля читаются не как остальные:
 ...
 *  - `ready` — дом сдан. Выгрузки пишут это то словом («ready»), то числом
 *    («1»), поэтому разбирается своей функцией `parseReadiness`, а не как
 *    обычное «да/нет». Пустое значение означает «неизвестно»: объявлять весь
 *    каталог строящимся из-за отсутствия поля нельзя.
 */
