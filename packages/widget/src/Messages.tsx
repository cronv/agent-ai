 ...
 *
 * ── Место для формы контакта (тикет 09) ─────────────────────────────────────
 *
 * Форма — такой же блок ленты, как сообщение: она встаёт последней, ниже всех
 * ответов и выше поля ввода, и прокручивается вместе с перепиской. Готовый
 * узел передаётся сюда через `contactSlot` — ниже отмечено, куда он попадает.
 * Ничего больше в этом файле для неё менять не нужно.
 */
...
interface MessagesProps {
  ...
  /** Тикет 09: сюда встаёт форма контакта. */
...
export function Messages({
  ...
  return (
    <div class="feed">
      ...
      {/* ТИКЕТ 09: форма контакта рендерится здесь, последним блоком ленты. */}
      {contactSlot}
    </div>
  )
}
